...
- **Registry** (`registry.js`): `styleBible` mirrors Motion Registry semantics — duplicate names throw at registration, unknown lookups return `null`, definitions deep-frozen. `listActive()` excludes `status: 'future'` styles (declared, validated, not offered).
- **Schema** (`schema.js`): `validateStyleDefinition` enforced at import time — shape checks, cross-references (real `renderStyle`; every motion preference must resolve to an IMPLEMENTED Motion Registry kind/preset of the right category, so a style can never steer the planner toward a static shot), and incompatible-option detection (a `forbidden` term appearing in the style's own vocabulary/anchors/prefix/suffix throws).
- **11 definitions** (`styles/`): 9 active — the production defaults `cinematic` / `paper-collage` (vox) / `luxury`, each codifying its render style's shipped look so defaults preserve existing behaviour, plus the cinematic family `documentary`, `ai-documentary`, `history`, `finance`, `modern-tech`, `minimal` — and 2 future (`pixar-style`, `anime`). Each defines philosophy, composition/framing/lighting rules, palette, camera behaviour + motion preferences (with per-style frequency guidance), typography, image-prompt medium/prefix/suffix/vocabulary/rules, forbidden elements, and consistency rules + prompt anchors.
- **Planner composed from data** (`prompts.js`): `shotsPrompt({ …, visual })` — the function owns the scaffolding (word indices, JSON contract, coverage rules); the definition owns every word of visual language. The motion vocabulary offered is narrowed from "everything implemented" to the style's own preferences (planner output still validated against the full registry in `validateShots`). The old `cameraPresetLegend`/`transitionLegend`/`effectLegend` sourcing was replaced by per-style `motionFieldLines(visual)`.
- **Consistency system** (`index.js` `composeImagePrompt`): THE single enforcement point for AI image requests — `prefix, <planner's shot description>, …promptAnchors, suffix`, comma-joined, trailing punctuation stripped, capped at 1200 chars. Called only in `pipeline.js`; `scenes.json` keeps the raw planner prompt. All shots of one video share medium/palette/grade descriptors — one film, not sixteen images.
- **Resolution** (`index.js`): `resolveVisualStyle(choice, renderStyle)` — active choice wins, else the render style's default; unknown/legacy/future values degrade to defaults, never crash. `defaultVisualStyle`: cinematic → cinematic, vox → paper-collage, luxury → luxury.
- **API + storage**: projects persist `visualStyle` (`store.js`; `getProject` backfills legacy projects with their render style's default); POST `/api/projects` accepts optional `visualStyle` (400 on unknown/future ids); project GET always reports it; `GET /api/options` gains `visualStyles` (9 options with `renderStyle` for picker filtering). Contract bumped to v2.1 in `docs/api-contract.md`; frontend types extended (`VisualStyleId`, `VisualStyleOption`, `visualStyle` on Project/CreateProjectPayload/OptionsResponse).
- **Smoke test** `scripts/test-stylebible.js` (**71 checks**, pure Node): registry semantics, schema acceptance/rejection (incl. cross-category motion refs + self-contradiction), every style's motion prefs re-verified implemented against the live registry, resolution fallbacks, options shape, `composeImagePrompt` layout, and full `shotsPrompt` composition for each active style (no `undefined`, only the style's own presets offered).
- **Validated**: Style Bible smoke **71/71**, motion smoke 159/159, pipeline test 17/17, baseline pixel-compare max per-channel delta 8/255 (grain/AA noise — legacy vox path untouched), fresh server + all endpoints healthy (options list, create-time 400s, explicit + default + legacy-backfill projects verified live), full re-render of `d2fe791fdb84` RENDER OK, frontend `tsc` + `vite build` clean, oxlint 1 pre-existing warning only.
