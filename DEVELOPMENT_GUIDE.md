...
- **Assets (Asset Engine)**: consumers NEVER build asset file paths or read the `assets/` tree themselves — resolve semantic ids (`sfx.paper.rip`, `music.documentary.calm`, `icon.money`) through `src/assets/index.js` (`resolveAsset`, `resolveAssetPath`, or the integration seams in `integration.js`). Adding an asset = drop a supported file into the right `Nerrico/assets/` folder (optionally curate via `tags.json`); adding a category = one entry in `ASSET_CATEGORIES` (schema.js). Registry must be populated via `initAssetEngine()` (explicit, awaited — not an import side effect). **Asset intelligence (4B)** lives in `intelligence.js` as pure data tables: a new style's asset flavor = one `STYLE_ASSET_PREFERENCES` entry; a new motion sound = one `MOTION_SFX_EVENTS` entry (or a per-style `motionSfx` override); music selection ALWAYS goes through `selectMusic()` (user → project → style → engine — never pick a track ad-hoc). **The provider (4C)** is the ONLY thing renderers see: compositions consume render-ready objects (`src` URL + `start`/`end`/`volume`/`loop`/`fadeIn`/`fadeOut`/`enabled`/`priority`) built by `provideMusic`/`provideSfx`/`provideIcon`/`buildAssetTimeline` — never pass semantic ids, records, or file paths into Remotion, and never tweak mix values inline (they live in `PROVIDER_MIX`). Library files are served ONLY via `GET /api/assets/:id/file` (exact ids). All values everywhere are semantic refs — never filenames. Run `node scripts/test-assets.js`, `test-assets-intelligence.js` AND `test-assets-provider.js` after any `src/assets/` change (plus `test-render-assets.js` with the server up when the render path changes). Full architecture: `docs/asset-engine.md`.
- **Voice Engine (NVE)**: voice selection ALWAYS goes through `selectVoice()` (user → project → mode+language → language default → engine fallback — never hardcode a voiceId in pipeline, renderer, or prompt). New voices are one `voiceRegistry.register({…})` block in `src/voice/definitions/<provider>.js`; new providers are one entry in `VOICE_PROVIDERS` (schema.js). Planner emits semantic refs (`voice.documentary`, `voice.realestate`) — never raw ElevenLabs voiceIds. Backwards compat: `resolveByProviderId()` maps old project.json raw voiceIds to VoiceRecords without breaking existing projects. Run `node scripts/test-voice-engine.js` after any `src/voice/` change. Full architecture: `docs/voice-engine.md`.
...
- **SFX/background music**: renders HAVE audio as of 4C — the `<AssetAudioLayer>` in `Short.jsx` plays the provider-built timeline (`inputProps.assets`). Never wire sounds into compositions directly: everything goes musicPlan/scene-fields → `buildAssetTimeline` → provider objects. The mix (volumes, fades, priorities) lives ONLY in `PROVIDER_MIX`. **Music licenses are unreviewed — blocking for YouTube publishing.**
